...
{
	$ofn=$_SESSION['ofn'];
		$sql="select * from docs where ".$ofn."a=0";
	$c=0;
	$message="";
	$rslt=$cnn->query($sql);
	echo "<center><table><tr><td><b>Id<b></td><td><b>Num<b></td><td><b>Name<b></td><td><b>Type<b></td><td><b>Link<b></td><td><b>Action(s)</b></td><td><b>Status</b></td></tr>";
	if($rslt->num_rows>0)
	{
		$c=$rslt->num_rows;
		
		$message="You have ".$c." pending verification(s).";
		...
		while($row=$rslt->fetch_assoc())
		{
			echo "<tr>";
				...
				echo "<td><a class='btn btn-outline-success btn-sm' role='button' href=accept.php?id=".$row['id']."&lvl=".$ofn.">   ACCEPT THIS</b></a> ";
				if($ofn>1)
				echo ("<a class='btn btn-outline-danger btn-sm' role='button' href=query.php?id=".$row['id']."&lvl=".$ofn."> REJECT THIS</a></p> </td>");
				else
					echo "</td>";

			if ($ofn<4) {
				if ($row[($ofn+1).'a']==-1) 
					echo "<td><br> <i>was REJECTED by upper office </i>: <b> ".$row[(1+$ofn).'q']." </b></td>";
	...
	$message="<h3>You have ".$c." pending verification(s).<h3>";
	echo $message;}
echo "</div>";
